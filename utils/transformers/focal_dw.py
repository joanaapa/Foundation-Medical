...
class FocalNet(nn.Module):
    ...
    def load_weights(self, pretrained_dict=None, pretrained_layers=[], verbose=True):
        ...
        #print(need_init_state_dict['patch_embed.proj.weight'])
...
class D2FocalNet(FocalNet):
    ...
    def forward(self, x):
        ...
        assert (
            x.dim() == 4
        ), f"Focal Transformer takes an input of shape (N, C, H, W). Got {x.shape} instead!"
        outputs = {}
        y = super().forward(x)
        for k in y.keys():
...
def focal_tiny(patch_size=4, img_size=[224], pretrained=False, checkpoint=None,**kwargs):
    kwargs['drop_path_rate'] = kwargs['drop_path_rate'] if 'drop_path_rate' in kwargs else 0.2    
    model = D2FocalNet(input_shape=img_size[0]) #patch_size=patch_size,embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], **kwargs)
    if pretrained:
        checkpoint = checkpoint #download_weights("focal_tiny", patch_size, kwargs['pretrained_type'])
...
if __name__ == '__main__':
    ...
    #model = D2FocalNet(224)
    checkpoint = torch.load('/home/adithya/storage/SEEM_Weights/seem_focalt_v2_processed.pt')
    model = focal_tiny(img_size=[1024],pretrained=True,checkpoint=checkpoint)
